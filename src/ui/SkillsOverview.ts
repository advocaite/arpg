...
type OpenArgs = {
  ...
  current: { primary?: string; secondary?: string; slots: (string | undefined)[]; passives: (string | undefined)[] }
  onUpdate: (next: { primary?: string; secondary?: string; slots: (string | undefined)[]; passives: (string | undefined)[] }) => void
  onClose?: () => void
...
export default class SkillsOverviewUI {
  ...
  open(args: OpenArgs): void {
    ...
    const current: { primary?: string; secondary?: string; slots: (string | undefined)[]; passives: (string | undefined)[] } = {
      primary: args.current.primary ?? (args as any).current.primaryRefId,
      secondary: args.current.secondary ?? (args as any).current.secondaryRefId,
    ...
    const makeSlot = (sx: number, sy: number, label: string, currentId?: string, onClick?: () => void) => {
      ...
      const text = this.scene.add.text(sx - 60, sy - 10, currentId ? label : 'Empty', { fontFamily: 'monospace', color: '#ffffff', fontSize: '12px' }).setScrollFactor(0)
      ...
      return [rect, icon, text] as Phaser.GameObjects.GameObject[]
    }
    ...
    nodes.push(...makeSlot(-w / 2 + 180, -h / 2 + 56, 'Primary', current.primary, () => this.skillsMenu.open({ slotIndex: 0, skills: skillsList, currentId: current.primary, disabledIds: [...baseDisabled(), ...(current.secondary ? [current.secondary] : [])], onAccept: (id?: string) => { current.primary = id; this.open({ ...args, current }) } } as any)))
    nodes.push(...makeSlot(-w / 2 + 180, -h / 2 + 116, 'Secondary', current.secondary, () => this.skillsMenu.open({ slotIndex: 1, skills: skillsList, currentId: current.secondary, disabledIds: [...baseDisabled(), ...(current.primary ? [current.primary] : [])], onAccept: (id?: string) => { current.secondary = id; this.open({ ...args, current }) } } as any)))
    ...
    for (let i = 0; i < 4; i++) {
      ...
      nodes.push(
        ...makeSlot(pos.x, pos.y, `Slot ${i + 1}`, current.slots[i], () => this.skillsMenu.open({ slotIndex: i, skills: skillsList, currentId: current.slots[i], disabledIds: dis, onAccept: (id?: string) => { current.slots[i] = id; this.open({ ...args, current }) } } as any))
      )
    }
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const text = this.scene.add.text(colX - 10, h / 2 - 90, current.passives[i] ? 'Passive' : 'Empty', { fontFamily: 'monospace', color: '#ffffff', fontSize: '12px' }).setScrollFactor(0)
      rect.on('pointerover', () => rect.setStrokeStyle(2, 0x66ccff, 1))
      rect.on('pointerout', () => rect.setStrokeStyle(1, 0x333333, 1))
